...
namespace NekoForWindows
{
    internal class Module
    {
        ...
        public static void ReadConfig(string path)
        {
            ...
            {
                // Get values
                string port = GetNodeValue(rootNode, "port");
                string socksPort = GetNodeValue(rootNode, "socks-port");
                string redirPort = GetNodeValue(rootNode, "redir-port");
                string mixedPort = GetNodeValue(rootNode, "mixed-port");
                string tproxyPort = GetNodeValue(rootNode, "tproxy-port");
                string mode = GetNodeValue(rootNode, "mode");
                string secret = GetNodeValue(rootNode, "secret");
                string externalController = GetNodeValue(rootNode, "external-controller");

                Properties.Settings.Default.ConfigPort = port;
                Properties.Settings.Default.ConfigSocks = socksPort;
                Properties.Settings.Default.ConfigRedir = redirPort;
                Properties.Settings.Default.ConfigMixed = mixedPort;
                Properties.Settings.Default.ConfigTProxy = tproxyPort;
                Properties.Settings.Default.ConfigMode = mode;
                Properties.Settings.Default.ConfigSecret = secret;
                Properties.Settings.Default.ConfigController = externalController;

                // Get the DNS node
                var dnsNode = rootNode.Children[new YamlScalarNode("dns")] as YamlMappingNode;
                if (dnsNode != null)
                {
                    string enhancedMode = GetNodeValue(dnsNode, "enhanced-mode");
                    Properties.Settings.Default.ConfigEnhanced = enhancedMode;
                }
            }
